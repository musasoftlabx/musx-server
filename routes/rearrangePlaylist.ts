...
export default function rearrangePlaylist(params: RearrangePlaylist) {
  ...
  DB.exec(
    `UPDATE playlistTracks SET position = ${from.pos} WHERE trackId = ${from.id} AND playlistId = ${playlistId}`
  );

  DB.exec(
    `UPDATE playlistTracks SET position = ${to.pos} WHERE trackId = ${to.id} AND playlistId = ${playlistId}`
  );
