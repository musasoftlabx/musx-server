...
export const dashboard = () => {
  ...
  const recentlyPlayed = DB.query(
    `SELECT DISTINCT path, trackId, title, albumArtist, artists, genre, year, track, rating, plays, bitrate, size, duration, format, channels, channelLayout, sampleRate, encoder, artwork, waveform, palette
     FROM plays
     INNER JOIN tracks
